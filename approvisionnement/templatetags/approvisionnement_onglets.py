...
@register.inclusion_tag('approvisionnement/_sortie_onglets.html', takes_context=True)
def sortie_onglets(context):
    request = context.get('request')
    url_name = ''
    if request is not None and getattr(request, 'resolver_match', None):
        url_name = request.resolver_match.url_name or ''
    nb_en_attente = LigneSortie.objects.filter(
        bon__statut=BonSortie.Statut.BROUILLON
    ).count()
    return {
        'nb_en_attente': nb_en_attente,
        'onglet_validation': 'validation' in url_name,
